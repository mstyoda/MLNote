...
这个定理说明了，如上随机算法RWM的期望regret的上界为$\mathcal{R}_{T} \leq 2 \sqrt{T \log N}$，接下来的定理说明如果仅用\textbf{期望regret}作为指标的话，RWM 已经达到了极限:
\begin{Thm}
    当$N = 2$，存在一个loss的随机loss vector序列，使得对于任意在线学习算法都有：$E[R_{T}] \geq \sqrt{T / 8}$。
\end{Thm}
\begin{proof}
    令$l_{t}$等概率从$(0,1)^T$和$(1,0)^T$中取得，不论是随机算法还是确定算法，都可以用$p_{t,i}$的形式来描述：
    \begin{equation}
        \begin{split}
            E[R_{t}] &= E[\mathcal{L}_{T}] - E[\mathcal{L}_{T}^{min}]\\
            &= \sum_{t = 1}^{T}{E[p_{t,1}l_{t,1} + p_{t,2}l_{t,2}]} - E[\mathcal{L}_{T}^{min}]\\
            &= T/2 - E[\mathcal{L}_{T}^{min}]\\
        \end{split}
    \end{equation}
    ...
    \begin{equation}
        \begin{split}
            \mathcal{L}_{T}^{min} &= \min\{\mathcal{L}_{T,1},\mathcal{L}_{T,2}\}\\
            &= \frac{1}{2}(\mathcal{L}_{T,1} + \mathcal{L}_{T,2} - |\mathcal{L}_{T,1} - \mathcal{L}_{T,2}|)\\
            &= T/2 - |L_{T,1} - T/2|
        \end{split}
    \end{equation}
    ...
    \begin{equation}
        \begin{split}
            E[R_{t}] &= E[|L_{T,1} - T/2|]\\
            &= E[|\sum_{t = 1}^{T}\frac{1 + \sigma_{t}}{2} - T/2|]\\
            &= E[|\sum_{t = 1}^{T}\frac{\sigma_{t}}{2}|]\\
        \end{split}
    \end{equation}
    
    使用书上的定理D.4，并且令$x_{t} = 1/2$得到：
    \begin{equation}
        E[R_{t}] = E[|\sum_{t = 1}^{T}{\sigma_{t}x_{t}}|] \geq \sqrt{\frac{1}{2}\sum_{t = 1}^{T}{x_{t}^2}} = \sqrt{T/8}
    \end{equation}
\end{proof}

如何使用这个定理去证明$R_{t} = \Omega( \sqrt{T\log N})$，\textbf{没有想出来}！！！。
